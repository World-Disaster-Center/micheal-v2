...
const AlertDrawer = ({ isOpen, onClose }) => {
  ...
  const { loading, error, data } = useSelector((state) => state.prediction);
  ...
  const handleSubmit = () => {
    ...
    dispatch(sendWeatherAlertRequest({ receiverEmail: email, cities: citiesArray }));
  };

  return (
    <CustomDrawer isOpen={isOpen} onClose={onClose} title="Send Weather Alerts" width="w-2/6">
      <div className="p-4 bg-gray-50 rounded-md shadow-md">
        ...
        <div className="mb-4">
          <label className="block text-gray-700 font-medium mb-2">Cities (comma-separated)</label>
          <input
            type="text"
            value={cities}
