...
const MapTypeModal = ({ isOpen, onClose, onSelectMapType, buttonRef }) => {
    ...
    return (
      <div 
        ref={modalRef}
        className="fixed z-50 bg-white rounded-lg shadow-lg overflow-visible w-48"
        style={{
          bottom: `${window.innerHeight - buttonRect.top + modalOffset}px`,
          left: `${buttonRect.left + (buttonRect.width - 192) / 2}px`
        }}
      >
        <div className="p-2">
        ...
        <div 
          className="absolute left bottom-0 w-3 h-3 bg-white transform rotate-45 translate-y-1.5 -translate-x-1.5"
          style={{ 
